-- Modifier la fonction initialize_new_school pour ne plus créer d'année académique
-- car elle sera créée manuellement avec les vraies dates de début et fin

CREATE OR REPLACE FUNCTION public.initialize_new_school(school_id_param uuid, school_type_param school_type DEFAULT 'public'::school_type, academic_year_name_param text DEFAULT '2024/2025'::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Ne plus créer l'année académique ici car elle est créée manuellement avec les vraies dates

...
import { ArrowLeft, FileText, Printer, Calculator, Eye } from "lucide-react";
...
export default function ResultatsSemestre() {
  ...
  useEffect(() => {
    // Récupérer les paramètres du système scolaire
    // Remplacé par hook Supabase
      // const savedSettings = // localStorage.getItem("schoolSettings") // Remplacé par hook Supabase;
    if (savedSettings) {
      const settings = JSON.parse(savedSettings);
      setSchoolSystem(settings.systemType || 'semestre');
    }
    ...
    if (classeId) {
      ...
      if (savedClasses) {
        ...
        // Récupérer les matières de cette classe - crucial pour les examens aussi
        // Remplacé par hook Supabase
        if (savedMatieres) {
          const allMatieres = JSON.parse(savedMatieres);
          const matieresDeClasse = allMatieres.filter((matiere: any) => matiere.classeId === classeId);
          setMatieresClasse(matieresDeClasse);
          } else {
          }
        ...
        if (savedEleves) {
          ...
          // Le classeId est l'ID numérique, mais eleve.classe stocke le nom complet (ex: "CI A")
          // Il faut trouver le nom complet de la classe à partir de son ID
          let elevesDeClasse = [];
          
          if (classeInfo) {
            const nomCompletClasse = `${classeInfo.session} ${classeInfo.libelle}`;
            elevesDeClasse = allEleves.filter((eleve: Student) => eleve.classe === nomCompletClasse);
            } else {
            }
          
          setEleves(elevesDeClasse);
          } else {
          }
      }
    }
